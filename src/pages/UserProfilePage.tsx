...
import photo from '../images/icon/UserIcon.png';
...
const UserProfilePage = () => {
  ...
  const [mblUserData, setMblUserData] = useState<any>({});
  const [activitiesData, setActivitiesData] = useState<any>([]);
  const [mandateData, setMandateData] = useState<any>([]);
  const [dbPhoto, setDbPhoto] = useState<any>('');
  const [showModal, setShowModal] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');

  const getMblAppUser = async () => {
    try {
      const apiUrl = `http://132.10.100.221:9912/webservices/users/mbldata/${userData?.PIN}`;
  ...
  const getActivitiesHistory = async () => {
    try {
      const apiUrl = `http://132.10.100.221:9912/webservices/users/mblserviceactivities/${userData?.PIN}`;
      const response = await axios.get(apiUrl);
  ...
  const getMandateData = async () => {
    try {
      const apiUrl = `http://132.10.100.221:9912/crusader/webservices/api/mandate/${userData?.PIN}`;
      const response = await axios.get(apiUrl);

      if (response.status === 200) {
        const data = response.data;
        setMandateData(data?.data[1][0]);
        getImages(data?.data[1][0].PICTURE_IMAGE.data);
      } else {
        console.error(
          `Failed to fetch user results. Status: ${response.status}`,
        );
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const apiUrl = `http://132.10.100.221:9912/webservices/users/${userId}`;
        const response = await axios.get(apiUrl);
  ...
  return (
    <>
      ...
      />
      <Breadcrumb
        pageName={`${capitalizeFirstLetter(
          userData?.FirstName,
        )} ${capitalizeFirstLetter(userData?.LastName)}`}
      />

      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="relative z-20 h-35 md:h-50 bg-main dark:bg-black">
          {/* <img
            src={CoverOne}
            alt="profile cover"
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div
            className={`relative rounded-full z-30 mx-auto -mt-22 h-30 w-full max-w-30 bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3`}
          >
            <div className="relative drop-shadow-2  h-[100%] w-[100%] rounded-full overflow-hidden  ">
              <img src={dbPhoto || photo} alt="profile" className="w-[100%]" />
            </div>
          </div>
          <div className="mt-4">
            <h3 className="mb-1.5 text-2xl font-semibold text-black dark:text-white">
              {`${
                mblUserData?.title ? mblUserData?.title : ''
              } ${capitalizeFirstLetter(
                userData?.FirstName,
              )} ${capitalizeFirstLetter(userData?.LastName)}`}
            </h3>
            <p className="font-medium">{userData?.PIN}</p>
            <div className="mx-auto mt-4.5 mb-5.5 grid max-w-203 grid-cols-3 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F]">
              ...
              <div className="flex flex-col items-center justify-center gap-1 border-r border-stroke px-4 dark:border-strokedark xsm:flex-row">
                ...
                <span className="text-sm font-semibold">
                  {formatDate(userData?.LastLoginTimeStamp)}
                </span>
              </div>
              <div className="flex flex-col items-center justify-center gap-1 px-4 xsm:flex-row">
                ...
                <span className="text-sm font-semibold">
                  {userData?.PIN ? 'Existing' : 'Prospect'}
                </span>
              </div>
            </div>
            ...
            <div className="flow-root rounded-lg border border-gray-2 dark:border-body py-3 shadow-sm max-w-150 mx-auto">
              <dl className="-my-3 divide-y divide-gray dark:divide-body text-sm text-left max-w-150">
                ...
                <div className="grid grid-cols-1 gap-1 p-3 sm:grid-cols-3 sm:gap-4">
                  <dt className="font-medium text-gray-900">Fund</dt>
                  <dd className="text-gray-700 sm:col-span-2">
                    {mblUserData?.fund_id
                      ? `Fund ${mblUserData?.fund_id}`
                      : 'No PIN'}
                  </dd>
                </div>
                ...
                <div className="grid grid-cols-1 gap-1 p-3 sm:grid-cols-3 sm:gap-4">
                  ...
                  <dd className="text-gray-700 sm:col-span-2">Encrypted</dd>
                </div>
              </dl>
            </div>
          </div>
      ...
      <div className="mt-8 rounded-sm border border-stroke bg-white px-5 py-4 pb-4 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-4">
        <div className="md:flex md:flex-row md:justify-between items-center">
          <p className="mt-7 md:mt-0">
            Ensure you've thoroughly examined all available methods before
            proceeding with a customer password reset.
          </p>
          <button
            onClick={() => {}}
            className="flex cursor-pointer items-center justify-center gap-2 rounded bg-primary py-1 px-2 text-sm font-medium text-white hover:bg-opacity-80 xsm:px-4"
