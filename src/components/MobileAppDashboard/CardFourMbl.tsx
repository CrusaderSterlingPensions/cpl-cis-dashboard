...
const CardFourMbl = () => {
  ...
  const fetchData = async () => {
    ...
    const response = await fetch(
      'http://132.10.100.221:9912/webservices/users/total-users',
    );
      ...
      <div className="mt-4 px-2 flex items-end justify-between">
        ...
        <span className="flex items-center gap-1 text-sm font-medium text-meta-3">
          <span className="flex flex-col justify-end items-end">
            <p>{`${resultData?.percentageIncrease}%`}</p>
            <p className=" text-xs font-bold">Last Month</p>
          </span>
          <svg
